...
const ListingCard: React.FC<ListingCardProps> = ({ listing, onClick, onRequestCall, callUsNumber }) => {
    ...
    >
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={(e) => {
            e.stopPropagation();
            onRequestCall && onRequestCall(listing);
          }}
          className="flex-1 bg-slate-900 text-white text-xs font-semibold py-2 rounded-lg hover:bg-slate-800 transition-colors"
        >
          Request call
        </button>
        <a
          ...
          className="flex-1 border border-slate-200 text-slate-700 text-xs font-semibold py-2 rounded-lg text-center hover:bg-slate-50 transition-colors"
        >
          Call us
        </a>
      </div>
