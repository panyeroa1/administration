...
interface ListingDetailsProps {
  ...
  onClose: () => void;
  onLoginRequest?: () => void;
  onRequestCall?: (listing: Listing) => void;
  callUsNumber?: string;
}

const ListingDetails: React.FC<ListingDetailsProps> = ({ listing, currentUser, onClose, onLoginRequest, onRequestCall, callUsNumber }) => {
            ...
            <div className="flex-1">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                        <button
                            onClick={() => onRequestCall && onRequestCall(listing)}
                            className="flex-1 bg-slate-900 text-white font-semibold py-2.5 rounded-lg hover:bg-slate-800 transition-colors"
                        >
                            Request call
                        </button>
                        <a
                            href={callUsNumber ? `tel:${callUsNumber.replace(/[^\d+]/g, '')}` : 'tel:+18444849450'}
                            className="flex-1 border border-slate-200 text-slate-700 font-semibold py-2.5 rounded-lg text-center hover:bg-slate-100 transition-colors"
                        >
                            Call us
                        </a>
                    </div>
